...
export const NotificationsProvider: React.FC<{
  ...
  const notificationsValue = useProvideNotifications(
    maxNotifications,
    defaultDuration,
    disableHistory
  );
...
function useProvideNotifications(
  ...
  const [state, setState] = useState<NotificationsState>(() => {
    ...
    const storedHistory =
      !disableHistory && typeof window !== 'undefined' ? localStorage.getItem(HISTORY_KEY) : null;
  ...
  const { connected, lastMessage, send, subscribe, unsubscribe } = useWebSocket({
  ...
  const removeNotification = useCallback((id: string): void => {
    setState(prev => {
      const notifications = prev.notifications.filter(notification => notification.id !== id);
