...
export const useAnalytics = (config: AnalyticsConfig = {}): UseAnalyticsReturn => {
  ...
  const sendAnalyticsData = useCallback((event: AnalyticsEvent) => {
    if (!isEnabled) return;
  ...
  const trackInteraction = useCallback((elementId: string, action: string, properties?: Record<string, any>) => {
    sendAnalyticsData({
      type: 'click',
