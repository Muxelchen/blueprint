...
export const subtractTime = (date: Date | string | number, amount: number, unit: TimeUnit): Date => {
...
export const getWeeksDifference = (date1: Date | string | number, date2: Date | string | number): number => {
  return Math.floor(getDaysDifference(date1, date2) / 7);
};

export const getMonthsDifference = (date1: Date | string | number, date2: Date | string | number): number => {
  const d1 = createDate(date1);
  const d2 = createDate(date2);
