...
export class DevUtils {
    ...
    const generators = {
      dashboard: () => ({
        ...
        recentActivity: Array.from({ length: count }, (_, i) => ({
          id: i + 1,
          user: `User ${i + 1}`,
          action: ['logged in', 'made purchase', 'updated profile', 'viewed product'][
      ...
      analytics: () => ({
        metrics: {
          ...
          avgSessionDuration: `${Math.floor(Math.random() * 5) + 2}:${Math.floor(Math.random() * 60)
            .toString()
            .padStart(2, '0')}`,
        },
  ...
  static generateLazyImports(components: string[]): string {
    return components
      .map(comp => `const ${comp} = lazy(() => import('./components/${comp}'));`)
      .join('\n');
  }
...
export const AppWithSuspense = () => (
  <Suspense fallback={<LoadingSpinner />}>
    <Router>
      <Routes>
        ${components
          .map(comp => `<Route path="/${comp.toLowerCase()}" element={<${comp} />} />`)
          .join('\n        ')}
      </Routes>
    </Router>
  </Suspense>
