...
const LineChart: React.FC<LineChartProps> = ({
  ...
  const getAdaptiveDimensions = () => {
    ...
    if (height) {
      ...
      gridColumns =
        containerHeight > 500 ? 'grid-cols-4' : activeLineCount > 2 ? 'grid-cols-3' : 'grid-cols-2';
    }
  ...
  return (
    ...
    >
      <div
        className={`flex flex-col sm:flex-row sm:justify-between sm:items-center ${dimensions.marginBottomChart} gap-2 flex-shrink-0`}
      >
        ...
        <div className={`flex items-center ${dimensions.spacing} justify-end`}>
          ...
          <span className={`${dimensions.fontSize} text-gray-500 whitespace-nowrap`}>
            {zoomDomain
              ? `${displayData.length} of ${data.length} periods`
              : `${data.length} periods`}
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0" style={{ height: dimensions.chartAreaHeight }}>
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            {!compact && (
              <Legend
                wrapperStyle={{ fontSize: dimensions.fontSize === 'text-xs' ? '10px' : '11px' }}
              />
            )}
            ...
            {showBrush && !compact && (
              <Brush dataKey="name" height={25} stroke="#8884d8" onChange={onBrushChange} />
            )}
          </RechartsLineChart>
        </ResponsiveContainer>
      </div>
      ...
      {showInstructions && !compact && (
        <div className={`mt-2 ${dimensions.fontSize} text-gray-500 text-center`}>
          {showBrush ? 'Use brush at bottom to zoom • ' : ''}Toggle lines above • Hover for details
          {showBrush ? ' • Click reset to zoom out' : ''}
        </div>
      )}
    </div>
  );
