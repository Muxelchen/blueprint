...
export class MockWebSocketServer {
  ...
  public registerConnection(socket: WebSocket, url: string): void {
    ...
    console.log(
      `[MockWebSocketServer] New connection, total connections: ${this.connections.size}`
    );
  }
  ...
  public removeConnection(socket: WebSocket): void {
    ...
    console.log(
      `[MockWebSocketServer] Connection closed, remaining connections: ${this.connections.size}`
    );
  }
  ...
  private handleSubscribe(socket: WebSocket, channel: string): void {
    ...
    console.log(
      `[MockWebSocketServer] Subscription to ${channel}, total subscribers: ${subs.size}`
    );
  ...
  private handleUnsubscribe(socket: WebSocket, channel: string): void {
    ...
    console.log(
      `[MockWebSocketServer] Unsubscribed from ${channel}, remaining subscribers: ${subs.size}`
    );
    ...
    switch (dataType) {
      ...
      case 'userActivity':
        this.sendToSocket(
          socket,
          'USER_ACTIVITY',
          this.generateUserActivity(parameters?.count || 10)
        );
        break;
  ...
  private startDataStreamForChannel(channel: string): void {
    ...
    switch (channel) {
      ...
      case 'notifications':
        ...
        this.timers.set(
          ...
          setInterval(() => {
            if (Math.random() > 0.7) {
              // Only send notifications sometimes
    ...
    console.log(
      `[MockWebSocketServer] Started data stream for channel: ${channel}, interval: ${interval}ms`
    );
  }
  ...
  private broadcastToChannel(channel: string, type: string, payload: any): void {
    ...
    console.log(
      `[MockWebSocketServer] Broadcast ${type} to ${sent}/${subscribers.size} subscribers in ${channel}`
    );
  }
  ...
  public broadcastToAll(type: string, payload: any): void {
    ...
    console.log(
      `[MockWebSocketServer] Broadcast ${type} to ${sent}/${this.connections.size} connections`
    );
  }
  ...
  public addListener(id: string, callback: (message: WebSocketMessage) => void): void {
    ...
    console.log(
      `[MockWebSocketServer] Added listener: ${id}, total listeners: ${this.listeners.size}`
    );
  }
  ...
  public removeListener(id: string): void {
    ...
    console.log(
      `[MockWebSocketServer] Removed listener: ${id}, remaining listeners: ${this.listeners.size}`
    );
  }
...
class MockWebSocket implements WebSocket {
  ...
  public addEventListener(
    ...
    if (typeof listener === 'function') {
      ...
      this.listeners[type].push((event: Event) =>
